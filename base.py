...
from utils import get_logger, ensure_path_exists, get_filename
from constants.midi import LOWEST_MIDI_NOTE, HIGHEST_MIDI_NOTE
...
class BaseTranscription(metaclass=ABCMeta):
    ...
    def _load_model(self, model_path=None, custom_objects=None):
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Checkpoint file not found: {model_path}/variables/variables.data*"
            )
    ...
    def _resolve_semi_feature_output_path(self, dataset_path, settings):
        if settings.dataset.feature_save_path == "+":
            base_output_path = dataset_path
            settings.dataset.save_path = dataset_path
        else:
            base_output_path = settings.dataset.feature_save_path
        train_feat_out_path = jpath(base_output_path, "semi_feature")
        ensure_path_exists(train_feat_out_path)
        return train_feat_out_path
...
class BaseDatasetLoader:
    ...
    def __init__(self, is_labeled = True, feature_folder=None, feature_files=None, num_samples=100, slice_hop=1, feat_col_name="feature"):
